...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const mailto = `mailto:sundarlohar@example.com?subject=${encodeURIComponent(form.subject)}&body=${encodeURIComponent(`From: ${form.name} (${form.email})\n\n${form.message}`)}`
    window.open(mailto)
    setSent(true)
        ...
        <div className="grid gap-10 lg:grid-cols-5">
          <div className="space-y-6 lg:col-span-2">
            <div className="flex items-start gap-4 rounded-xl border border-border bg-background/60 backdrop-blur-sm dark:bg-background/80 dark:backdrop-blur-sm p-5">
              <Mail className="mt-1 size-5 shrink-0 text-blue-400" />
              <div>
                <p className="text-sm font-medium">Email</p>
                <a
                  href="mailto:sundarlohar@example.com"
                  className="text-sm text-muted-foreground transition-colors hover:text-blue-400"
                >
                  sundarlohar@example.com
                </a>
              </div>
            </div>

            <div className="flex items-start gap-4 rounded-xl border border-border bg-background/60 backdrop-blur-sm dark:bg-background/80 dark:backdrop-blur-sm p-5">
              <MapPin className="mt-1 size-5 shrink-0 text-blue-400" />
              <div>
            ...
            <div className="flex items-start gap-4 rounded-xl border border-border bg-background/60 backdrop-blur-sm dark:bg-background/80 dark:backdrop-blur-sm p-5">
              <Globe className="mt-1 size-5 shrink-0 text-blue-400" />
              <div>
                <p className="text-sm font-medium">LinkedIn</p>
                <a
                  href="https://linkedin.com/in/sundarlohar"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm text-muted-foreground transition-colors hover:text-blue-400"
                >
                  linkedin.com/in/sundarlohar
                </a>
              </div>
            </div>
          </div>
